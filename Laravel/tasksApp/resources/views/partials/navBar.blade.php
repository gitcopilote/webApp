                    ...
                    @foreach (Auth::user()->notifications as $notification)
                        ...
                        @unless ($notification->read_at)
                            @if (isset($notification->data['task_id']))
                                {{-- Si la notification concerne une tâche --}}
                                <a href="{{ route('task.show', ['task' => $notification->data['task_id']]) }}"
                                    ...
                                    <div class="w-full pl-3">
                                        <div class="text-gray-500 text-sm mb-1.5 dark:text-gray-400">
                                            ...
                                            {{-- <span class="font-medium text-blue-500" href="#">{{ $notification->data['name'] }}</span> --}}
                                            {{-- <span class="font-medium text-blue-500" href="#">{{ $notification->data['status'] }}</span> --}}
                                        </div>
                            ...
                            @elseif (isset($notification->data['historique']))
                                {{-- Ajouter une autre condition ici --}}
                                <a href="{{ $notification->data['historique'] }}"
                                    class="flex px-4 py-3 hover:bg-gray-100 dark:hover:bg-gray-700">
                                    <div class="w-full pl-3">
                                        <div class="text-gray-500 text-sm mb-1.5 dark:text-gray-400">
                                            <span class="font-semibold text-gray-900 dark:text-white">
                                            ...
                                            {{-- <span class="font-medium text-blue-500">{{ $notification->data['message'] }}</span> --}}
                                        </div>
                                        <div class="text-xs text-blue-600 dark:text-blue-500">
                                            {{ $notification->created_at->diffForHumans() }}
                                        </div>
                            ...
                            @else
                                {{-- Sinon, si la notification concerne une autre action --}}
